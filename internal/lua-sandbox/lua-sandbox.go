...
// Lua sandbox helper functions and wrapper functions of provided Go APIs
var helpers = `
...
// runCommand parses the function and runs the function, only if it is supported
func goExecute(l *lua.LState) int {
	...
	// TODO: Make args parsing more robust
...
func getGitRoot() string {
	...
	if err != nil || strings.TrimSpace(string(output)) != "true" {
		...
		if gitDir == "." || gitDir == ".git" {
			// If the .git directory is the current directory then the root
	...
	// Get the root directory of the git repository if the current directory is
	// already inside the working tree
...
func goScanDir(l *lua.LState) int {
	...
	if recursive {
		...
		// Return all files in the current directory without entering
		// subdirectories
